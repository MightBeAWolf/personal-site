...
- `site/src/content/glossary/*.md` — one file per hover-glossary term (frontmatter: `title`, `summary`, optional `link`, optional `aliases` list). Each entry renders two ways from one source: a dedicated page at `/glossary/<slug>` (`site/src/pages/glossary/[slug].astro`) and, fetched and its `#glossary-content` extracted, inside a hover popup (`site/src/components/GlossaryPopup.astro`). `title`/`aliases` are matched against `tech-stack.astro`'s copy by `site/src/lib/glossary-link.ts` to decide which words become triggers — drop a new file in to add a term, nothing else needs to change.
- `site/src/content/practices/*.md` + `site/src/data/tech-stack.ts` — data behind `/tech-stack`'s three-column layout (category label | sub-item + description | accordion detail). `practices` is a content collection (frontmatter: `section` (`tools`|`workflows`), `title`, `teaser`, `order`) whose markdown body is the accordion's expanded content for the Tools/Workflows sections, rendered with `render()` same as glossary entries. `tech-stack.ts` exports a plain `StackCategory[]` tree (Category → Group → Leaf) for the Tech-stack section specifically — structural data (labels + optional glossary cross-refs), not prose, so it's a TS module rather than a content collection. A leaf's `glossaryId`, when set, must match a real `glossary` entry id; `tech-stack.astro` reuses that entry's summary/link as the leaf's content instead of duplicating it.
